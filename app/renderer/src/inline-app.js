...
function iconSvg(name, size = 16, opts = {}) {
...
// (#131) via window.grading, so the pill and the letter grade can never
// disagree — the classifiers themselves live in grading.js.
function pillLabel(tone) { return tone === 'good' ? 'Good' : tone === 'check' ? 'Check' : tone === 'issue' ? 'Issue' : 'Info'; }
function pillIcon(tone) { return tone === 'good' ? 'check' : tone === 'issue' ? 'x' : tone === 'info' ? 'info' : 'alert-triangle'; }
function statusPillHTML(tone, label) {
  return `<span class="pill sm ${tone}">${iconSvg(pillIcon(tone), 11, { strokeWidth: 2.25 })}${label || pillLabel(tone)}</span>`;
}

function gradeRingHTML(grade, score) {
  const px = 168, stroke = 8;
  const r = (px - stroke) / 2;
  const c = 2 * Math.PI * r;
  const dash = Math.max(0, Math.min(100, score)) / 100 * c;
  // grade can now come from a disk-stored history record (#147), not just the
  // app's own computeGrade() output — strip to letters only for the CSS custom
  // property name and escape separately for display, so a crafted record can't
  // break out of the style attribute or inject markup as element content.
...
function fmtDev(d) { return (d >= 0 ? '+' : '') + d.toFixed(1) + ' dB'; }
function deviationMiniCurve(dev) {
...
function renderProfileMatch(profile, cmp) {
  const el = document.getElementById('rc-profile');
  const label = escapeHtml(profile.label);
  let regions = '';
  ...
  el.innerHTML = `
    <div class="rcp-head">
      <span class="rcp-name">Target: ${label}${idealProfileId ? '' : ' <span style="color:var(--text-muted)">(auto)</span>'}</span>
...
function renderReportCardFromHistory(summary) {
  ...
  document.getElementById('rc-recommendations').innerHTML = (summary.topFixes || []).map(r => {
    ...
    return `<div class="rc-rec${critical ? ' critical' : ''}">
      ...
      <span class="rc-rec-text">${critical ? '<span class="rc-rec-badge">Critical</span>' : ''}${escapeHtml(text)}</span>
    </div>`;
  }).join('');
...
function renderReportCard() {
  ...
  const rt = document.getElementById('rc-rec-type');
  rt.className = 'rc-rectype pill ' + recType.tone;
  rt.innerHTML = `${iconSvg(pillIcon(recType.tone), 13)}<span class="txt">${recType.label} — ${recType.note}</span>`;
  ...
  // grading.rcMetricTarget so a threshold change moves the displayed target with
  // the grade. Metrics with no target in config (Clipping), or with no measured
  // value (DR/Centroid absent), get null → the row renders an explicit "—".
  const metrics = [
    { name: 'Peak Level', note: 'Sample peak', value: fmt(src.peak), unit: 'dBFS', tone: grading.rcPeakStatus(src.peak, src.clipping), target: grading.rcMetricTarget('peak') },
    { name: 'RMS Level', note: 'Average level (RMS)', value: fmt(src.rms), unit: 'dBFS', tone: grading.rcRmsStatus(src.rms), target: grading.rcMetricTarget('rms') },
    { name: 'Dynamic Range', note: src.dynamicRange != null ? null : 'Not measured for live capture', value: src.dynamicRange != null ? fmt(src.dynamicRange) : '—', unit: src.dynamicRange != null ? 'dB' : '', tone: grading.rcDrStatus(src.dynamicRange), target: src.dynamicRange != null ? grading.rcMetricTarget('dynamicRange') : null },
    { name: 'Clipping', value: src.clipping ? 'Yes' : 'None', unit: '', tone: src.clipping ? 'issue' : 'good', target: grading.rcMetricTarget('clipping') },
    { name: 'Spectral Centroid', value: src.centroid ? Math.round(src.centroid).toLocaleString() : '—', unit: src.centroid ? 'Hz' : '', tone: grading.rcCentroidStatus(src.centroid), target: src.centroid ? grading.rcMetricTarget('centroid') : null },
  ];
  document.getElementById('rc-metrics-body').innerHTML = metrics.map(m =>
    `<tr>
  ...
  // Why this grade (#133) — the per-deduction breakdown, straight from the pure
  // grading module so it can never disagree with the letter it explains. Each
  // deduction names the rule, its measured value vs. the config target, and the
  // letter impact; an empty list renders an explicit positive "no deductions"
  // state (never a blank box).
  const explain = grading.explainGrade(src);
  const why = document.getElementById('rc-why');
  // Clipping is the only rule that forces an automatic F, and when it fires it is
  // the sole deduction (grading short-circuits) — so the authoritative
  ...
  // #136 — disclose any grading rule that was skipped because its metric wasn't
  // measured (live captures have no dynamic range). Rendered as a neutral "info"
  // disclosure, not a deduction: it never dropped a letter, but the grade used
  // fewer metrics and the user must be told rather than left to assume the DR
  // rule ran. This is what keeps a live grade from silently diverging from a
  // file grade. The rows come straight from the pure grading module, so they
  // can never claim a skip the grade didn't take.
  ...
  if (deductionRows.length === 0 && notMeasuredRows.length === 0) {
    ...
    // No deductions among the rules that ran, but at least one rule was skipped —
    // lead with the honest positive, then disclose what wasn't measured. Never
    // the unqualified "met every grading rule" (a rule didn't run).
    why.innerHTML =
      `<div class="rc-why-none">
        <span class="rc-why-icon">${iconSvg('check', 18)}</span>
    ...
    why.innerHTML = deductionRows.join('') + notMeasuredRows.join('');
  }
  ...
  // Recommendations
  document.getElementById('rc-recommendations').innerHTML = grading.computeRecommendations(src).map(r => {
    const critical = r.startsWith('CRITICAL');
    const text = critical ? r.replace(/^CRITICAL:\s*/, '') : r;
    return `<div class="rc-rec${critical ? ' critical' : ''}">
      <span class="rc-rec-icon">${iconSvg(critical ? 'alert-triangle' : 'check', 16)}</span>
      <span class="rc-rec-text">${critical ? '<span class="rc-rec-badge">Critical</span>' : ''}${text}</span>
    </div>`;
  }).join('');
...
/* ══ Guided path to grading a real service (#142) ══ */
