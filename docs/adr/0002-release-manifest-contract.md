...
`.app` as `Contents/Resources/app-update.yml`. `scripts/release.sh` stages
`latest-mac.yml` on the draft release alongside `latest.json`, before
promote — the same safety property this ADR established for `latest.json`
(a draft is never `releases/latest`, so a half-published release never
advertises itself). The CI release workflow (`.github/workflows/release.yml`)
is a separate, simpler backup path: it has no draft/promote staging and
never uploads `latest.json` at all — it publishes only the zip and, now,
`latest-mac.yml`, directly and non-draft, whenever `RELEASES_TOKEN` is
configured. `scripts/release.sh` remains the only path that publishes a
complete, safety-staged release. `app/electron/auto-updater.ts`
