# Carve-out compliance gate for the Troubleshooting section ships as a verification-only pass because the feature already landed
...
scanning the eight #839 section files for AI-configuration and network tokens,
...
the merged and already design-reviewed tests for zero user-visible gain. This is
the same replay situation that #381/#834/#835/#836/#837/#838 resolved with
ADR-0032–0037, now applied to the final #839 story. The BUILD pass therefore
must not re-implement; it verifies and fixes only concrete regressions in place.

## Decision

Drive issue #864's BUILD pass as verification-only: run the issue's stated
verification commands (npm run lint --prefix app; npm test --prefix app; npm
test) plus the workspace build gate (npm run build), confirm every gate is green
and each acceptance criterion holds against the merged gate tests, and make zero
code changes unless a specific gate genuinely fails (then fix that regression in
place, never re-port the gate surface). Leave
...
factory's recorded replay convention (ADR-0025/0026/0032–0037) is extended to
the final #839 story. Negative: if a gate fails on an already-merged state, the
